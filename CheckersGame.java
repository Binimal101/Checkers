...
public class CheckersGame {
	...
	public String gameloop() throws IOException { // Returns the path of the game to delete out of scope
		...
		while (true) {
			...
			// Too many turns since last jump
			if ((turnsSinceLastJump / 2) == STALEMATEDELIMITER) { // div2 as each player should have moved 40 times
				...
				// turn cannot commence as there is nowhere to go
			} else if (!board.canMove(currentPlayer.charAt(0)) && !board.canJump(currentPlayer.charAt(0))) {
				clearConsole();
				System.out.println(board);
				...
				if (Math.abs(dx - x) == 2 && Math.abs(dy - y) == 2) { // We know that the move is valid and two spaces
																		// away, we are jumping
					turnsSinceLastJump = 0;

					int[] middlePosition = board.getIntermediaryPosition(x, y, dx, dy);
					board.move(new int[] { x, y }, middlePosition); // Short stopping at our middle position to kill
																	// enemy
					board.move(middlePosition, new int[] { dx, dy }); // from short stop to final position

					String[] jumpingNames = new String[] { "FILLER", "DOUBLE", "TRIPLE", "QUADRUPLE", "QUINTUPLE",
							"SEXTUPLE", "SEPTUPLE", "OCTUPLE", "N-TUPLE", "DOU-DECUPLE" };
	...
	public int[] getProperCoordinates(String currentPlayer) {
		...
		do { // Continues until player has decided upon correct coordinates
			...
			while (!withinBounds(x, y) || board.isNullAt(x, y) || !isAllowedToControl(currentPlayer, x, y)) {
				...
				if (move == null) { // Move will return null if player enters ""
					continue;
				}
			...
			while (!withinBounds(dx, dy)) {
				moveTo = getMoveFromPlayer(currentPlayer, "Move to"); // moveTo will return null if player enters ""
	...
	public int[] getMoveFromPlayer(String player, String askingFor) {
		...
		while (!Character.isDigit(tryX.charAt(0))) {
			...
			System.out.println("(Enter nothing to restart)");
			...
			if (tryX.isEmpty()) {
				clearConsole();
				return null; // will resume loop in outer scope
			} else {
				clearConsole();
			}
		...
		while (!Character.isDigit(tryY.charAt(0))) {
			...
			if (tryY.isEmpty()) {
				clearConsole();
				return null; // Will resume in loop
			} else {
				clearConsole();
			}
